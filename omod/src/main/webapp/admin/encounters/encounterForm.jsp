            ...
            <td>
                <div id="patient_editable" >
                    <spring:bind path="encounter.patient">
                        <openmrs_tag:patientField formFieldName="patientId" searchLabelCode="Patient.find" initialValue="${encounter.patient.patientId}" linkUrl="${pageContext.request.contextPath}/admin/patients/patient.form" callback="updateSaveButtonAndVisits" allowSearch="${encounter.encounterId == null}"/>
        ...
        <tr>
            ...
            <td>
                <spring:bind path="encounter.location">
                    <openmrs_tag:locationField formFieldName="location" initialValue="${encounter.location}"/>
                    <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                </spring:bind>
            </td>
        </tr>
            ...
            <spring:bind path="encounter.visit">
                <select id="visitSelect" name="${status.expression}">
                    ...
                    <c:forEach items="${patientVisits}" var="visit">
                        <option value="${visit.visitId}" <c:if test="${visit.visitId == encounter.visit}">selected="selected"</c:if>>
                        ...
                        <c:when test="${encounter.encounterId == null}">
                            ...
                            <select name="encounterType">
                                <c:forEach items="${encounterTypes}" var="type">
                                    ...
                                    <option value="${type.encounterTypeId}" <c:if test="${type.encounterTypeId == encounter.encounterType.encounterTypeId}">selected</c:if>><c:out value="${type.name}"/></option>                              
                                </c:forEach>
                                <c:if test="${groupOpen}">
                                    </optgroup>
                                </c:if>
                        ...
                        <c:when test="${encounter.encounterId == null}">
                            <select name="form">
                                ...
                                <c:forEach items="${forms}" var="form">
                                    <option value="${form.formId}" <c:if test="${form.formId == encounter.form.formId}">selected</c:if>><c:out value="${form.name}"/></option>
                                </c:forEach>
                            </select>
                        </c:when>
                        <c:otherwise>
            ...
            <tr id="voidReason">
                ...
                <td>
                    <spring:bind path="encounter.voidReason">
                        <input type="text" value="${encounter.voidReason}" name="${status.expression}" size="40" />
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
            </tr>
