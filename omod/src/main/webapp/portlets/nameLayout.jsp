                        ...
                        <c:if test="${model.layoutShowExtended == 'true'}">
                            <td>
                                <spring:bind path="preferred">
                                    ...
                                    <input type="checkbox" name="${status.expression}" onclick="preferredBoxClick(this)" alt="personName" value="true" <c:if test="${preferred == true}">checked</c:if> />
                ...
                <c:otherwise>
                    ...
                    <c:if test="${model.layoutShowExtended == 'true'}">
                            <tr>
                                ...
                                <td>
                                    <spring:bind path="preferred">
                                        <input type="hidden" name="_${status.expression}">
                                        <input type="checkbox" name="${status.expression}" onclick="if (preferredBoxClick) preferredBoxClick(this)" alt="personName" value="true" <c:if test="${preferred == true}">checked</c:if> />
                                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                                    </spring:bind>
                                </td>
                            </tr>
                    ...
                    <c:if test="${model.layoutShowExtended == 'true'}">
                            <spring:bind path="creator">
                                <c:if test="${!(creator == null)}">
                                    <tr>
                                        ...
                                        <td colspan="4">
                                            <c:out value="${creator.personName}"/> -
                            ...
                            <spring:bind path="changedBy">
                                <c:if test="${!(changedBy == null)}">
                                    <tr>
                                        <td><openmrs:message code="general.changedBy" /></td>
                                        <td colspan="4">
                                            <c:out value="${changedBy.personName}"/> -
                                    ...
                                    <td>
                                        <spring:bind path="voided">
                                            ...
                                            <input type="checkbox" name="${status.expression}" 
                                                   <c:if test="${voided == true}">checked="checked"</c:if>
                                                   onClick="toggleLayer('<spring:bind path="personNameId">voidReasonNameRow-${personNameId}</spring:bind>'); if (voidedBoxClicked) voidedBoxClicked(this); "
                                            />
                                        </spring:bind>
                                    </td>
                                </tr>
                                <tr id="<spring:bind path="personNameId">voidReasonNameRow-${personNameId}</spring:bind>"
                                    style="<spring:bind path="voided"><c:if test="${voided == false}">display: none;</c:if></spring:bind>">
                                    <td><openmrs:message code="general.voidReason"/></td>
                                    <spring:bind path="voidReason">
                                        <td colspan="4">
                                            <input type="text" name="${status.expression}" value="${voidReason}" size="43" />
                                            <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                                        </td>
                                    </spring:bind>
                                </tr>
                                <spring:bind path="voidedBy">
                                    <c:if test="${!(voidedBy == null)}">
                                        <tr>
                                            <td><openmrs:message code="general.voidedBy" /></td>
                                            <td colspan="4">
                                                <c:out value="${voidedBy.personName}"/> -
